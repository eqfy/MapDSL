...
export function convertCoordinateToLatLng(position: Position): google.maps.LatLngLiteral {
...
export function convertTileToCoordinate(tileCoordinate: google.maps.Point, zoom: number): Position {
...
export function isTileInCanvas(coord: Position): boolean {
...
export function isCoordinateInCanvas(coord: Position): boolean {
  return coord.x >= 0 && coord.x <= Constants.CANVAS_WIDTH && coord.y >= 0 && coord.y <= Constants.CANVAS_HEIGHT;
}
